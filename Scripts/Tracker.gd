...
func calculate_result():
	...
	var PURCHASE = (POPULARITY * (1 - (GAMEPRICE * 0.01)) * GAMEREPUTATION)
	var v3 = PURCHASE * GAMEPRICE
	var v4 = 0
	if MICROTRANSACTIONS:
		var subject = float(PURCHASE) * randf_range(0.1, 0.2)
		v4 = calcular_ganancias_masivas(subject, 3)
	if DATACOLLECT:
		v4 += calcular_ganancias_masivas(PURCHASE, 5)
	if ADS:
		v4 += calcular_ganancias_masivas(PURCHASE, 4)
...
func calcular_ganancias_masivas(usuarios: float, curva: float) -> int:
	...
	for i in range(10):
		muestra_promedio += pow(randf(), curva) * 100.0
